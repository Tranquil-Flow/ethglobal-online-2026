...
function normalizedSet(values, name) {
  if (
    ...
    !values.length ||
    values.some((x) => typeof x !== "string" || !x)
  )
    fail("INVALID_PROXY_CONFIG", name);
  return new Set(values.map((x) => x.toLowerCase()));
}
function duplicates(rawHeaders) {
  const counts = new Map();
  for (let i = 0; i < rawHeaders.length; i += 2) {
    const n = rawHeaders[i].toLowerCase();
...
export function createHttpsProxy({
  ...
  try {
    ...
    fail("INVALID_PROXY_CONFIG", "upstream");
  }
  if (
    ...
    fail("INVALID_PROXY_CONFIG", "upstream must be a loopback origin");
  if (!cert || !key)
    fail("INVALID_PROXY_CONFIG", "explicit cert and key required");
  ...
  if (
    ...
    fail("INVALID_PROXY_CONFIG", "allowedPaths");
  for (const [v, n] of [
    ...
    if (!Number.isSafeInteger(v) || v < 1) fail("INVALID_PROXY_CONFIG", n);
  ...
  const handler = (req, res) => {
    ...
    if (duplicates(req.rawHeaders))
      return send(res, 400, "duplicate security header");
    ...
    if (!req.url?.startsWith("/") || req.url.startsWith("//"))
      return send(res, 400, "invalid target");
    let parsed;
    try {
      parsed = new URL(req.url, target);
    } catch {
      return send(res, 400, "invalid target");
    }
    if (parsed.origin !== target.origin || req.url.includes("\\"))
      return send(res, 400, "invalid target");
    const pathAllowed = allowedPaths.some((p) =>
      p.endsWith("/") ? parsed.pathname.startsWith(p) : parsed.pathname === p,
    );
    const readiness = parsed.pathname === "/readyz";
    if (readiness && req.method !== "GET")
      return send(res, 405, "method not allowed");
    if (!pathAllowed && !readiness) return send(res, 404, "not found");
    const length = req.headers["content-length"];
    if (
      length !== undefined &&
      (!/^\d+$/.test(length) || BigInt(length) > BigInt(maxBodyBytes))
    )
    ...
    if (readiness) {
      ...
      parsed = new URL("/healthz", target);
    }
    const transport = target.protocol === "https:" ? httpsRequest : httpRequest;
    const upstreamReq = transport(
      parsed,
      {
        method: readiness ? "GET" : req.method,
        headers,
          ...
          upstreamRes.on("end", () =>
            send(
              ...
              upstreamRes.statusCode >= 200 && upstreamRes.statusCode < 300
                ? 200
                : 503,
              upstreamRes.statusCode < 300 ? "ready" : "upstream unavailable",
            ),
          );
          return;
        }
        for (const [name, value] of Object.entries(upstreamRes.headers))
          if (!HOP.has(name) && name !== "set-cookie" && value !== undefined)
    ...
    active.add(upstreamReq);
    upstreamReq.once("close", () => active.delete(upstreamReq));
    upstreamReq.on("timeout", () =>
      upstreamReq.destroy(Error("UPSTREAM_TIMEOUT")),
    );
    upstreamReq.on("error", () => {
      if (!res.headersSent) send(res, 503, "upstream unavailable");
      else res.destroy();
    });
    let bytes = 0,
      rejected = false;
    ...
    if (readiness) upstreamReq.end();
    else pipeline(req, upstreamReq, () => {});
  };
  ...
  return {
    async listen({ host = "127.0.0.1", port = 0 } = {}) {
      ...
      if (
        ...
        fail("INVALID_LISTEN_CONFIG", "loopback host and valid port required");
      server = createServer(
        {
          cert,
          key,
      ...
      await new Promise((ok, no) =>
        server.listen(port, host, ok).once("error", no),
      );
      const a = server.address();
      return {
        url: `https://${host.includes(":") ? `[${host}]` : host}:${a.port}`,
      };
    },
