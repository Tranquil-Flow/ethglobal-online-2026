...
export function createGraphClient({endpoint,token,fetch:fetchImpl=globalThis.fetch,allowLocal=false,maxBytes=2*1024*1024}){
 let url;try{url=new URL(endpoint);}catch{throw failure('INVALID_GRAPH_CONFIG');}
 const local=['127.0.0.1','[::1]','localhost'].includes(url.hostname);
 if(url.username||url.password||url.search||url.hash||!(url.protocol==='https:'||(allowLocal&&local&&url.protocol==='http:'))||!Number.isSafeInteger(maxBytes)||maxBytes<1024||maxBytes>4*1024*1024||token&&(!/^[\x21-\x7e]+$/.test(token)||token.length>4096))throw failure('INVALID_GRAPH_CONFIG');
 return {async query({query,variables={},signal}){
  const sig=deadline(signal,5000);
  try{
   const response=await bounded(fetchImpl(url,{method:'POST',redirect:'error',signal:sig,headers:{'content-type':'application/json',...(token?{authorization:`Bearer ${token}`}:{})},body:JSON.stringify({query,variables})}),sig);
   if(!response.ok||!response.body)throw failure('GRAPH_UNAVAILABLE',true);
   const reader=response.body.getReader();let bytes=0,chunks=[];
   try{for(;;){const {done,value}=await bounded(reader.read(),sig);if(done)break;bytes+=value.length;if(bytes>maxBytes)throw failure('GRAPH_BOUNDS');chunks.push(value);}}
   finally{await reader.cancel().catch(()=>{});}
   const body=JSON.parse(Buffer.concat(chunks).toString('utf8'));
   if(body.errors?.length||!body.data)throw failure('GRAPH_UNAVAILABLE',true);
   return body.data;
  }catch(e){if(e?.retryable!==undefined)throw e;throw failure('GRAPH_UNAVAILABLE',true);}
 }};
}
export function historyReasons(history,{trustedVerifiers=[]}={}){
 ...
 const reasons=[];
 if(history.observations.some(a=>!trustedVerifiers.includes(a.verifierId)))reasons.push('UNKNOWN_VERIFIER');
 if(history.observations.some(a=>a.outcome==='mismatch'))reasons.push('ASSESSMENT_MISMATCH');
 if(history.observations.some(a=>a.outcome==='unavailable'||a.outcome==='inconclusive'||a.outcome==='pending'))reasons.push('ASSESSMENT_UNAVAILABLE');
 return reasons.length?reasons:['HISTORY_OBSERVED'];
}
function historyConfig(config){
 if(!modes.includes(config.mode)||!/^\d{1,16}$/.test(String(config.chainId)))throw failure('INVALID_HISTORY_CONFIG');
 const maxAgeMs=config.maxAgeMs??300000,limit=config.limit??100,timeoutMs=config.timeoutMs??5000;
 if(!Number.isSafeInteger(maxAgeMs)||maxAgeMs<1||maxAgeMs>86400000||!Number.isSafeInteger(limit)||limit<1||limit>1000||!Number.isSafeInteger(timeoutMs)||timeoutMs<10||timeoutMs>30000)throw failure('INVALID_HISTORY_CONFIG');
...
export async function queryProviderHistory({config,client,provider,providerId,signal}){
 ...
 const report={history,reasons:[],counts:{},provenance:[],truncated:false};
 if(client&&c.deployment){
  ...
  try{
   ...
   for(const row of data.assessmentClaims){
    ...
    if(seen.has(row.objectDigest))continue;seen.add(row.objectDigest);
    observations.push(a);provenance.push({objectDigest:digest(row.objectDigest),transactionHash:row.transactionHash,blockNumber:Number(row.blockNumber),blockHash:row.blockHash,logIndex:Number(row.logIndex),publisher:row.publisher,contractAddress:row.contractAddress,chainId:String(row.chainId)});
    const key=JSON.stringify([a.verifierId,a.outcome]);counts[key]=(counts[key]||0)+1;
   }
   history.observations=observations;history.indexedBlock=meta.block.number;history.indexedBlockHash=meta.block.hash;
   history.freshness=now-meta.block.timestamp*1000>c.maxAgeMs?'stale':'fresh';
   report.provenance=provenance;report.counts=counts;report.truncated=data.assessmentClaims.length===c.limit;
  }catch(e){if(signal?.aborted)throw failure('ABORTED',true);report.failureCode=e?.code==='ABORTED'?'HISTORY_TIMEOUT':'HISTORY_UNAVAILABLE';}
 }
 validate('History',history);report.reasons=historyReasons(history,c);if(report.truncated)report.reasons.push('HISTORY_WINDOW_LIMIT');return report;
}
