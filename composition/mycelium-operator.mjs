...
const fail = (code = "INVALID_OPERATOR_INPUTS") => {
  throw Error(code);
};
...
export function validateOperatorInputs(input) {
  ...
  closed(input, [
    ...
    "runtimeProfile",
    "providers",
    ...(unchecked ? [] : ["replayGateway"]),
    "expectedEvidenceClass",
    "access",
  ]);
  if (
    ![
      "mycelium.workbench.operator.v1",
      "mycelium.workbench.operator.v2",
    ].includes(input.schema) ||
    !/^[a-f0-9]{40}$/.test(input.upstreamCommit) ||
    !text(input.expectedEvidenceClass) ||
  ...
  if (
    m.mode !== "live" ||
    r.runtime.execution_kind !== "model" ||
    m.runtime.sourceCommit !== input.upstreamCommit ||
    m.model.id !== r.model_id ||
    m.model.revision !== r.resolved_commit ||
    m.codec.tokenizerDigest !== r.codec.tokenizer_digest ||
    m.codec.templateDigest !== r.codec.template_digest ||
    m.qualification.manifestDigest !== r.manifest_digest ||
    m.limits.maxOutputTokens > r.max_new_tokens_limit ||
    m.limits.maxPromptCharacters > 256 ||
    m.limits.maxPromptUtf8Bytes > 1024 ||
    m.limits.maxOutputTokens > 64 ||
    !p.profile.artifacts.some(
      (a) =>
        a.role === "mycelium-runtime-profile-v1" && a.digest === digestOf(r),
    )
  )
    fail("OPERATOR_PROFILE_MISMATCH");
  if (
    !Array.isArray(input.providers) ||
    input.providers.length < 1 ||
    input.providers.length > 8 ||
    new Set(input.providers.map((g) => g.providerId)).size !==
      input.providers.length
  )
    fail();
  for (const g of [
    ...input.providers,
    ...(unchecked ? [] : [input.replayGateway]),
  ]) {
    closed(g, [
      ...(g === input.replayGateway ? [] : ["providerId"]),
      "baseUrl",
      "credentialRef",
      "qualification",
      "evidenceClass",
    ...
    origin(g.baseUrl);
    if (
      !/^[A-Za-z0-9._-]{1,128}$/.test(g.credentialRef) ||
      g.evidenceClass !== input.expectedEvidenceClass ||
      (g !== input.replayGateway && !text(g.providerId))
    )
      fail();
    // The production constructor validates the exact native binding, without I/O.
    createGatewayV3SessionFactory({
      transport: {
        qualification() {
          fail();
        },
      ...
      workbenchProfileId: p.profileId,
      runtimeProfile: r,
  ...
  if (
    m.qualification.qualificationDigest !== primary.qualification_digest ||
    m.qualification.deploymentId !== primary.deployment_id ||
    m.qualification.epoch !== String(primary.deployment_epoch)
  )
    fail("OPERATOR_QUALIFICATION_MISMATCH");
  if (
    !unchecked &&
    input.providers.some((g) => g.baseUrl === input.replayGateway.baseUrl)
  )
    fail("SEPARATE_REPLAY_GATEWAY_REQUIRED");
  ...
  if (
    ...
    ![
      a.maxPrimaryRequests,
      ...(unchecked ? [] : [a.maxReplayRequests]),
      a.maxOutputTokens,
      a.concurrency,
    ].every(positive) ||
    (unchecked && a.maxReplayRequests !== 0) ||
    a.concurrency !== 1 ||
    a.maxOutputTokens > m.limits.maxOutputTokens ||
    !Array.isArray(a.primaryOrigins) ||
    a.primaryOrigins.some((x) => origin(x) !== x) ||
    [...new Set(a.primaryOrigins)].sort().join(",") !==
      input.providers
        .map((g) => g.baseUrl)
        .sort()
        .join(",") ||
    (!unchecked && a.replayOrigin !== input.replayGateway.baseUrl)
  )
    fail("INVALID_ACCESS_SCOPE");
  return Object.freeze({
    mode: "live",
    inputDigest: digestOf(input),
    profileId: p.profileId,
    runtimeProfileId: digestOf(r),
    upstreamCommit: input.upstreamCommit,
    providerIds: input.providers.map((g) => g.providerId),
    accessReference: a.reference,
    expiresAt: a.expiresAt,
    networkContacted: false,
    grantVerified: false,
    ...(input.schema.endsWith(".v2")
      ? {
          checking: unchecked
            ? "unavailable"
            : "configured-replay-not-qualified",
        }
      : {}),
  });
}
...
export async function createOperatorRuntimeBinding(
  ...
  if (
    typeof authorizeRuntimeAccess !== "function" ||
    typeof credentialFor !== "function"
  )
    fail("RUNTIME_ACCESS_GRANT_REQUIRED");
  if (
    (await authorizeRuntimeAccess({
      inputDigest: pins.inputDigest,
  ...
  const runtime = await createMyceliumRuntimeBinding({
    ...
    timeoutMs: 60000,
    providers: await Promise.all(input.providers.map(gateway)),
    ...(input.replayGateway
      ? { replayGateway: await gateway(input.replayGateway) }
      : {}),
  });
  return guardOperatorRuntime(
    { ...runtime, bindingDigest: pins.inputDigest },
    { ...pins, access: input.access },
  );
}
...
export function guardOperatorRuntime(runtime, { inputDigest, access }) {
  ...
  return {
    ...
    create(context) {
      ...
      if (initial && initial.inputDigest !== inputDigest)
        fail("ACCESS_SCOPE_CHANGED");
      if (!initial)
        store.set("operator-grants", id, {
          inputDigest,
          primary: 0,
          replay: 0,
        });
      ...
      function check(request) {
        ...
        if (request && request.maxOutputTokens > access.maxOutputTokens)
          fail("ACCESS_OUTPUT_LIMIT");
      }
      function reserve(kind, request) {
        ...
        store.transaction(() => {
          ...
          if (
            row[kind] >=
            access[
              kind === "primary" ? "maxPrimaryRequests" : "maxReplayRequests"
            ]
          )
            fail("ACCESS_BUDGET_EXCEEDED");
      ...
      const bounded = (args) => ({
        ...
        signal: AbortSignal.any([
          ...
          AbortSignal.timeout(
            Math.max(
              1,
              Math.min(Date.parse(access.expiresAt) - Date.now(), 60000),
            ),
          ),
        ]),
      });
      return {
        ...
        executor: {
          ...
          validateRequest(request) {
            check(request);
            return ports.executor.validateRequest(request);
          },
          async *execute(args) {
            reserve("primary", args.request);
            try {
              yield* ports.executor.execute(bounded(args));
            } finally {
              active--;
            }
          },
        },
        ...(ports.assessor && access.maxReplayRequests > 0
          ? {
              assessor: {
                ...ports.assessor,
                async assess(args) {
                  reserve("replay");
                  try {
                    return await ports.assessor.assess(bounded(args));
                  } finally {
                    active--;
                  }
                },
