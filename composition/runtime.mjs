...
import { canonicalBytes, digestOf, validate } from "../packages/contracts/index.mjs";
...
export const simulatorProfile = deepFreeze({
  ...
  numerics: {
    ...
    determinism: "NFC code points, bounded integer reductions, and canonical SHA-256; not model execution",
  },
});
...
function stagedResult(request, divergent = false) {
  ...
  const codePoints = Array.from(normalized, (character) => character.codePointAt(0));
  const inputStage = {
  ...
  for (let index = 0; index < codePoints.length; index++) {
    weightedChecksum = (weightedChecksum + codePoints[index] * (index + 1)) % 4_294_967_291;
  ...
  const output = {
    ...
    finishReason: characters.length < Array.from(rendered).length ? "length" : "stop",
  };
  const outputStage = { outputHash: digestOf(output), tokenCount: output.tokenIds.length };
...
export function createSimulator({
  ...
  return Object.freeze({
    ...
    async *execute({ jobId, request, profile, signal } = {}) {
      ...
      if (digestOf(profile) !== digestOf(simulatorProfile) || request.profileId !== digestOf(profile))
        throw safeError("PROFILE_MISMATCH");
      if (fault === "worker-unavailable") throw safeError("WORKER_UNAVAILABLE");
      if (fault === "timeout") {
        await delay(2_147_483_647, undefined, { signal }).catch(() => { throw abortError(); });
        return;
      }
      if (fault === "malformed") {
      ...
      const eventCount = Math.ceil(result.output.tokenIds.length / chunkTokens) + 1;
      ...
      yield {
        ...
        profileId: fault === "profile-mismatch" ? ZERO_DIGEST : digestOf(profile),
...
function assessment({ bundle, method, verifierId, outcome, reasonCode, evidenceDigest }) {
  const receipt = bundle?.receipt;
  const receiptDigest = (() => {
    try { return digestOf(receipt); } catch { return ZERO_DIGEST; }
  })();
  const profileId = /^sha256:[0-9a-f]{64}$/.test(receipt?.payload?.profileId ?? "")
...
async function replay({ bundle, pins, reexecutor, signal, method, verifierId }) {
  checkSignal(signal);
  const unavailable = (reasonCode) => assessment({ bundle, method, verifierId, outcome: "unavailable", reasonCode });
  let corePins;
  try {
    corePins = pinsForCore(pins);
    if (pins.providerId !== bundle?.receipt?.payload?.providerId || pins.keyId !== bundle?.receipt?.keyId)
  ...
  if (!reexecutor || reexecutor.mode !== "development" || typeof reexecutor.execute !== "function")
  ...
  if (!integerOption(timeoutMs, 1, 30_000) || !integerOption(maxEvents, 2, 8192) || !integerOption(maxOutputBytes, 1, 1_048_576))
  ...
  try {
    ...
    for (let count = 0; count < maxEvents; count++) {
      ...
      try {
        const aborted = new Promise((_, reject) => {
          ...
          removeAbort = () => controller.signal.removeEventListener("abort", onWorkerAbort);
      ...
      if (event?.type === "delta" && typeof event.text === "string" && Array.isArray(event.tokenIds)) {
        if (digestOf(event.tokenIds) !== digestOf(Array.from(event.text, (character) => character.codePointAt(0))))
          return unavailable("TOKEN_TEXT_MISMATCH");
        text += event.text;
        tokenIds.push(...event.tokenIds);
        if (Buffer.byteLength(text) > maxOutputBytes || tokenIds.length > bundle.request.maxOutputTokens)
          return unavailable("REPLAY_LIMIT");
      } else if (event?.type === "completed") completed = event;
      else return unavailable("MALFORMED_REPLAY");
    }
    if (signal?.aborted) throw abortError();
    if (timedOut || controller.signal.aborted) return unavailable("REPLAY_TIMEOUT");
    ...
    if(!validOutput || completed.profileId!==bundle.receipt.payload.profileId || text!==completed.output.text || digestOf(tokenIds)!==digestOf(completed.output.tokenIds) || !/^sha256:[0-9a-f]{64}$/.test(completed.evidenceDigest??""))return unavailable("INVALID_REPLAY");
    const matches =
      validOutput &&
    ...
    return assessment({
      ...
      ...(matches ? { evidenceDigest: completed.evidenceDigest } : {}),
    });
  } catch (error) {
    if (signal?.aborted || error?.name === "AbortError" && signal?.aborted) throw abortError();
    return unavailable(controller.signal.aborted ? "REPLAY_TIMEOUT" : "WORKER_UNAVAILABLE");
  } finally {
    clearTimeout(timer);
    controller.abort();
...
export function replayEvidence({ bundle, pins, reexecutor = createSimulator(), signal } = {}) {
  return replay({ bundle, pins, reexecutor, signal, method: DEFAULT_METHOD, verifierId: DEFAULT_VERIFIER });
}
...
export function createReplayAssessor({
  ...
  if (typeof loadEvidence !== "function" || !/^[A-Za-z0-9_.:-]{1,256}$/.test(method) || !/^[A-Za-z0-9_.:-]{1,256}$/.test(verifierId))
  ...
  return Object.freeze({
    ...
    async assess({ receipt, profile, evidenceRef, signal } = {}) {
      ...
      if (!/^core-local:[a-f0-9-]{36}$/.test(evidenceRef ?? ""))
        return assessment({ bundle: shell, method, verifierId, outcome: "unavailable", reasonCode: "INVALID_EVIDENCE_REFERENCE" });
      let bundle;
      try {
        bundle = await loadEvidence(evidenceRef, { signal });
      } catch (error) {
        if (signal?.aborted || error?.name === "AbortError") throw abortError();
        return assessment({ bundle: shell, method, verifierId, outcome: "unavailable", reasonCode: "EVIDENCE_UNAVAILABLE" });
      }
      try {
        if (digestOf(bundle?.receipt) !== digestOf(receipt) || digestOf(bundle?.profile) !== digestOf(profile))
          return assessment({ bundle: shell, method, verifierId, outcome: "unavailable", reasonCode: "REQUEST_BINDING_MISMATCH" });
      } catch {
        return assessment({ bundle: shell, method, verifierId, outcome: "unavailable", reasonCode: "INVALID_EVIDENCE" });
      }
