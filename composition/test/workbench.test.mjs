...
test(
  "normal workbench: actual staged replay -> indexed mismatch -> different provider",
  { timeout: 180000 },
  async () => {
    ...
    try {
      ...
      async function proposal() {
        ...
        const quotes = await Promise.all(requests.map((r) => c.createQuote(r)));
        return {
          providers,
          requests,
          quotes,
      ...
      try {
        ...
        await page.screenshot({
          path: "artifacts/closeout/workbench-browser.png",
          fullPage: true,
        });
      } finally {
        await browser.close();
      ...
      await writeFile(
        ...
        JSON.stringify(
          {
            execution: "staged-simulation-not-inference",
            actualEns: true,
            actualGraph: true,
