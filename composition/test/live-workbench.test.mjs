...
import { createSigner, developmentProfile } from "../../packages/core/src/index.mjs";
...
function publicationSigner() {
  return {
    provider: { send() {}, getCode() {}, estimateGas() {}, getFeeData() {}, getTransactionCount() {}, getTransactionReceipt() {}, broadcastTransaction() {}, getBlock() {} },
    signTransaction() {},
    getAddress() {},
  };
}
function runtime(overrides = {}) {
  return {
    ...
    assessor: { mode: "live", async assess() { throw Error("CONTROLLED_NOT_ASSESSED"); } },
...
function config(dataDir, overrides = {}) {
  const base = {
    ...
    indexing: {
      deployment: {
        mode: "live", chainId: 11155111, network: "sepolia",
        address: "0x3333333333333333333333333333333333333333",
        publisher: "0x4444444444444444444444444444444444444444",
        startBlock: 1, confirmations: 12, codeHash: "0x" + "55".repeat(32),
      },
      graph: {
      ...
      publication: { enabled: true, maxGasPriceWei: "100000000000", timeoutMs: 1_000 },
...
test("fails closed before creating state or listeners for incomplete live authority", async () => {
  ...
  const cases = [
    [{ config: good, receiptSigner: signer, publicationSigner: publisher }, /LIVE_RUNTIME_REQUIRED/],
    [{ config: good, runtime: runtime({ mode: "development" }), receiptSigner: signer, publicationSigner: publisher }, /LIVE_RUNTIME_REQUIRED/],
    [{ config: good, runtime: runtime({ profiles: [] }), receiptSigner: signer, publicationSigner: publisher }, /RUNTIME_PROFILE_CATALOG_MISMATCH/],
    [{ config: good, runtime: runtime(), publicationSigner: publisher }, /RECEIPT_SIGNER_REQUIRED/],
    [{ config: good, runtime: runtime(), receiptSigner: signer }, /PUBLICATION_SIGNER_REQUIRED/],
    [{ config: config(dataDir, { payment: { ...good.payment, maxTotalAmountBaseUnits: undefined } }), runtime: runtime(), receiptSigner: signer, publicationSigner: publisher }, /INVALID_LIVE_CONFIG/],
    [{ config: config(dataDir, { payment: { ...good.payment, network: "hedera:mainnet" } }), runtime: runtime(), receiptSigner: signer, publicationSigner: publisher }, /INVALID_LIVE_CONFIG/],
    [{ config: config(dataDir, { indexing: { ...good.indexing, deployment: undefined } }), runtime: runtime(), receiptSigner: signer, publicationSigner: publisher }, /INVALID_LIVE_CONFIG/],
    [{ config: { ...good, surprise: true }, runtime: runtime(), receiptSigner: signer, publicationSigner: publisher }, /UNKNOWN_LIVE_CONFIG_FIELD/],
  ];
  for (const [options, expected] of cases) {
    await assert.rejects(startLiveWorkbench(options), expected);
    assert.equal(existsSync(dataDir), false, "validation must precede state creation");
  }
...
test("starts real loopback core with live factories, keeps HTTPS identity, and cleans up", async () => {
  ...
  const app = await startLiveWorkbench({
    config: config(dataDir), runtime: runtime(), receiptSigner: signer,
    publicationSigner: publicationSigner(), paymentAuthorizer: async () => null,
  });
  ...
  assert.deepEqual(health, { status: 200, body: { status: "ok", mode: "live" } });
  const session = await call(app.url, "/v1/sessions", { method: "POST", body: {} });
  assert.equal(session.status, 201);
  const request = {
    version: "1", nonce: "a".repeat(64), providerId: "worker.example.eth", profileId,
    prompt: "CONTROLLED_PRIVATE_TEST", maxOutputTokens: 8, seed: 0,
    sampling: "greedy", publishConsent: false,
  };
  const quote = await call(app.url, "/v1/quotes", { method: "POST", body: { request }, capability: session.body.capability });
  assert.equal(quote.status, 201);
  assert.equal(quote.body.amountBaseUnits, "9");
  const job = await call(app.url, "/v1/jobs", {
    method: "POST", body: { request, quoteId: quote.body.quoteId },
    capability: session.body.capability, key: "controlled-private-job",
  });
  assert.equal(job.status, 402, "real live PaymentsPort must require protocol authorization");
